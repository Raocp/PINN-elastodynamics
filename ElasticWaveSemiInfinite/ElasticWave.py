...
class DeepHPM:
    ...
    def __init__(self, Collo, SRC, IC, UP, uv_layers, lb, ub, ExistModel=0, modelDir=None):
        ...
        # tf graphs
        self.u_pred, self.v_pred, self.ut_pred, self.vt_pred, self.s11_pred, self.s22_pred, self.s12_pred = self.net_uv(
            self.x_tf, self.y_tf, self.t_tf)
        self.e11_pred, self.e22_pred, self.e12_pred = self.net_e(self.x_tf, self.y_tf, self.t_tf)

        self.u_IC_pred, self.v_IC_pred, self.ut_IC_pred, self.vt_IC_pred, _, _, _ = self.net_uv(self.x_IC_tf,
                                                                                                self.y_IC_tf,
                                                                                                self.t_IC_tf)
        ...
        self.f_pred_u, self.f_pred_v, self.f_pred_ut, self.f_pred_vt, self.f_pred_s11, self.f_pred_s22, self.f_pred_s12 = self.net_f_sig(
            self.x_c_tf, self.y_c_tf, self.t_c_tf)
        ...
        self.loss = 5 * self.loss_f_uv + 5*self.loss_f_s + 2*self.loss_IC + 2*self.loss_SRC + 2*self.loss_NB
    ...
    def train(self, iter, learning_rate, batch_num):
        ...
        for i in range(batch_num):
            ...
            tf_dict = {self.x_c_tf: self.x_c[idx_start:idx_end, :], self.y_c_tf: self.y_c[idx_start:idx_end, :],
                       self.t_c_tf: self.t_c[idx_start:idx_end, :],
...
def postProcess(xmin, xmax, ymin, ymax, field=[], s=12, num=0):
    ...
    ax[1, 0].set_xticks([])
    ax[1, 0].set_yticks([])
    ax[1, 0].set_xlim([xmin+45, xmax+45])
    ax[1, 0].set_ylim([ymin+45, ymax+45])
    # cf.cmap.set_under('whitesmoke')
    # cf.cmap.set_over('black')
    ...
    ax[1, 1].set_xticks([])
    ax[1, 1].set_yticks([])
    ax[1, 1].set_xlim([xmin+45, xmax+45])
    ax[1, 1].set_ylim([ymin+45, ymax+45])
    cf.cmap.set_under('whitesmoke')
    cf.cmap.set_over('black')
    ...
    ax[1, 2].set_xticks([])
    ax[1, 2].set_yticks([])
    ax[1, 2].set_xlim([xmin+45, xmax+45])
    ax[1, 2].set_ylim([ymin+45, ymax+45])
    # cf.cmap.set_under('whitesmoke')
    # cf.cmap.set_over('black')
    ...
    ax[1, 1].set_yticks([])
    ax[1, 1].set_xlim([xmin+45, xmax+45])
    ax[1, 1].set_ylim([xmin+45, xmax+45])
    # cf.cmap.set_under('whitesmoke')
    # cf.cmap.set_over('black')
    ...
    ax[1, 2].set_yticks([])
    ax[1, 2].set_xlim([xmin+45, xmax+45])
    ax[1, 2].set_ylim([xmin+45, xmax+45])
    # cf.cmap.set_under('whitesmoke')
    # cf.cmap.set_over('black')
...
def GenCirclePT(xc, yc, r, N_PT):
    ...
    theta = np.linspace(0, 2*np.pi, N_PT)
...
if __name__ == "__main__":
    ...
    ################Pre-train####################
    ### Note: please pretrain the network, such as with a smaller MAX_T=8, then extend to MAX=16
    PI = math.pi
    MAX_T = 16.0  # change this to 8 if you want train the network from scratch

    # Domain bounds
    lb = np.array([-15, -15, 0.0])
    ub = np.array([ 15,  15, MAX_T])
    ...
    xy_IC = np.array([-15, -15]) + np.array([30, 30]) * lhs(2, 12000)
    t_IC = 0*xy_IC[:,0:1]
    IC = np.concatenate((xy_IC, t_IC), 1)

    # Upper boundary point
    xt_up = np.array([-15, 0]) + np.array([30, MAX_T]) * lhs(2, 15000)
    y_up = 0*xt_up[:,0:1]
    y_up.fill(15.0)
    UP = np.concatenate((xt_up[:,0:1], y_up, xt_up[:,1:2]), 1)

    # Collocation point
    XYT_c = lb + (ub - lb) * lhs(3, N_f)
    XYT_c_ext1 = np.array([xc_src - r_src-2, yc_src - r_src -2, 0.0]) + np.array([2*(r_src+2), 2*(r_src+2), MAX_T]) * lhs(3, 15000)   # Refinement around source
    XYT_c_ext2 = np.array([-15, 15-6, 0.0]) + np.array([30, 6, MAX_T]) * lhs(3, 20000)
    XYT_c = np.concatenate((XYT_c, XYT_c_ext1, XYT_c_ext2),axis=0)
    XYT_c = DelSrcPT(XYT_c, xc=xc_src, yc=yc_src, r=r_src)

    print('XYT_c:',XYT_c.shape)
    print('UP:', UP.shape)
    print('IC:', IC.shape)

    # Curved collocation point
    # XYT_c = lb + (ub - lb) * lhs(3, 30000)
    # Y = XYT_c[:, 1]
    # Y = -(np.exp(Y/15)-np.exp(1))/(np.exp(1)-np.exp(-1))*30-15
    # XYT_c[:,1] = Y
    # XYT_c = DelSrcPT(XYT_c, xc=xc_src, yc=yc_src, r=r_src)
    ...
    ts = 3.0
    Amp = 1.0
    xx, yy = GenCirclePT(xc=xc_src, yc=yc_src, r=r_src, N_PT=150)   # N_PT=500
    # adjust the sampling density if you want
    tt = np.concatenate((np.linspace(0, 6, 153), np.linspace(6, MAX_T, 63)),axis=0)
    tt = tt[1:]
    ...
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    ax.scatter(XYT_c[:,0:1], XYT_c[:,1:2], marker='o', alpha=0.1, s=3)
    ax.scatter(SRC[:, 0:1], SRC[:, 1:2], marker='o', alpha=0.2, color='red')
    ax.scatter(UP[:, 0:1], UP[:, 1:2], marker='o', alpha=0.2,color='orange')
    plt.show()
    ...
    with tf.device('/device:GPU:0'):
        ...
        # Option 1: to train from start
        # model = DeepHPM(XYT_c, SRC, IC, UP, uv_layers, lb, ub)
        # Option 2: to read the trained network and perform prediction
        model = DeepHPM(XYT_c, SRC, IC, UP, uv_layers, lb, ub, ExistModel=1, modelDir='uv_NN#16s.pickle')

        start_time = time.time()
        # loss_f_uv, loss_f_s, loss_IC, loss_SRC, loss = model.train(iter=2000, learning_rate=5e-4, batch_num=1)
        model.train_bfgs(batch_num=1)
        print("--- %s seconds ---" % (time.time() - start_time))

        model.getloss()

        model.save_NN('uv_NN#16s-float64.pickle')
        ...
        # xy_IC = np.array([-15, -15]) + np.array([30, 30]) * lhs(2, 8000)
        # t_IC = 0 * xy_IC[:, 0:1]
        # IC = np.concatenate((xy_IC, t_IC), 1)
        #
        # # Upper boundary point
        # xt_up = np.array([-15, 0]) + np.array([30, MAX_T]) * lhs(2, 12000)
        # y_up = 0 * xt_up[:, 0:1]
        # y_up.fill(15.0)
        # UP = np.concatenate((xt_up[:, 0:1], y_up, xt_up[:, 1:2]), 1)
        #
        # # Collocation point
        # XYT_c = lb + (ub - lb) * lhs(3, N_f)
        # XYT_c_ext1 = np.array([xc_src - r_src - 2, yc_src - r_src - 2, 0.0]) + np.array(
        #     [2 * (r_src + 2), 2 * (r_src + 2), MAX_T]) * lhs(3, 10000)  # Refinement around source
        # # XYT_c_ext2 = np.array([-15, 15 - 6, 0.0]) + np.array([30, 6, MAX_T]) * lhs(3, 15000)
        # # XYT_c = np.concatenate((XYT_c, XYT_c_ext1), axis=0)
        # XYT_c = DelSrcPT(XYT_c, xc=xc_src, yc=yc_src, r=r_src)
        #
        # print('XYT_c:', XYT_c.shape)
        # print('UP:', UP.shape)
        ...
        # y_SRC = y_SRC.flatten()[:, None]
        # t_SRC = t_SRC.flatten()[:, None]
        # amplitude = Amp * (2 * PI ** 2 * (t_SRC - ts) ** 2 / tsh ** 2 - 1) * np.exp(
        #     -PI ** 2 * (t_SRC - ts) ** 2 / tsh ** 2)
        # u_SRC = amplitude * (x_SRC - xc_src) / r_src
        # v_SRC = amplitude * (y_SRC - yc_src) / r_src
        # SRC = np.concatenate((x_SRC, y_SRC, t_SRC, u_SRC, v_SRC), 1)
        #
        ...
        # ##############End of fine-tuning####################
        #
        # model.save_NN('uv_NN#16s.pickle')
        #
        #
        #
        ...
        for i in range(N_t):
            ...
            amp_pred = (u_pred**2 + v_pred**2)**0.5
            postProcess(xmin=-15, xmax=15, ymin=-15, ymax=15, s=5, field=field, num=i)
